      ...
      <div className='flex w-full xl:flex-col items-center justify-between
        ...
        {navData.map((link, index) => {
          return <Link 
                  ...
                  relative flex items-center group houver:text-accent hover:text-accent transition-all duration-300`}
                  href={link.path} 
                  key={index}
                >
                
                <div className='absolute pr-14 right-0 hidden xl:group-houver:flex'>
                  <div className='bg-white relative flex text-primary w-16 h-16 items-center p-[6px] rounded-[3px]'>
                    <div className='text-[12px] leading-none font-semibold capitalize'>
                      {link.name}
                    </div>

                    <div className='border-solid border-y-transparent border-y-[6px] border-r-0 absolute -right-2'>
